    ...
    public class RitualsLearn : SkillRoll
    {
        ...
        public int Roll(Build actor, Rite rite, bool hasSpec, bool hasWill)
        {
            ...
            if (result > 0)
            {
                ...
                {
                    //not enought yet!
                    Log.Log(Verbosity.Critical, string.Format("{0} has advanced in learning rite {1} on {2} more success. Now he has {3} of {4} successes!",
                        actor.Name, riteName, result, successesLearned, successesRequired));
                }
                else
                {
                    Log.Log(Verbosity.Critical, string.Format("{0} has has finally learned rite {1} (+{2} more success. Now he has {3} of {4} successes!)",
                        actor.Name, riteName, result, successesLearned, successesRequired));
            ...
            {
                Log.Log(Verbosity.Critical, string.Format("{0} didn't learn anything.", actor.Name));
            }
